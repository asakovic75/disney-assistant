...
3.  **ТОЧНАЯ ФИЛЬТРАЦИЯ:** При фильтрации (например, "фильмы 16+"), находи ТОЧНОЕ совпадение.
4.  **ПРЯМОЙ ОТВЕТ:** Не используй вводные фразы вроде "Согласно данным".

ФОРМАТ ОТВЕТА (ОБЯЗАТЕЛЬНО):
Твой ответ ДОЛЖЕН состоять из двух четко разделенных блоков: `[РАССУЖДЕНИЯ]` и `[ОТВЕТ]`.
-   **В блоке `[РАССУЖДЕНИЯ]`:** Опиши свой мыслительный процесс шаг за шагом. Укажи, какие именно строки из 'Данных' ты используешь для ответа. Если ты фильтруешь данные, объясни, по какому критерию ты это делаешь.
-   **В блоке `[ОТВЕТ]`:** Сформулируй финальный, чистый ответ для пользователя без лишних слов.
...
ОТВЕТ В СТРОГОМ ФОРМАТЕ:"""
                ...
                try:
                    reasoning_part, final_answer = answer.split("[ОТВЕТ]")
                    reasoning_text = reasoning_part.replace("[РАССУЖДЕНИЯ]", "").strip()
                    final_answer_text = final_answer.strip()
                except ValueError:
                    reasoning_text = "Модель не предоставила рассуждения в ожидаемом формате."
                ...
                st.markdown(f'<div class="big-answer-text">{final_answer_text}</div>', unsafe_allow_html=True)
                
                with st.expander("Показать ход мыслей Пикселя 🧠"):
